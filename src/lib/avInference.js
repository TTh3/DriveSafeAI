...
function getRuleLink(countryCode, maneuverKey) {
  const rules = {
    AU: { // South Australia
      ...
      default: { url: 'https://mylicence.sa.gov.au/my-car-licence/the-drivers-handbook', name: "Driver's Handbook", desc: 'Ensure you follow all posted signs, signals, and speed limits on the road.' }
    },
    US: { // California DMV
    ...
    GB: { // UK Highway Code
      ...
      default: { url: 'https://www.gov.uk/browse/driving/highway-code-and-safety', name: "Highway Code", desc: 'Be considerate to other road users and follow the Highway Code at all times.' }
    },
    JP: { // Japan
    ...
    DE: { // Germany
      roundabout: { url: 'https://www.bmdv.bund.de/SharedDocs/EN/publications/road-traffic-regulations-stvo.html', name: 'Roundabouts (StVO §8)', desc: 'Traffic in the roundabout has right of way. Do not indicate when entering, only when exiting.' },
      merge: { url: 'https://www.bmdv.bund.de/SharedDocs/EN/publications/road-traffic-regulations-stvo.html', name: 'Zipper Merge (StVO §7)', desc: 'Use the zipper method (Reißverschlussverfahren) to merge smoothly when a lane ends.' },
      turn: { url: 'https://www.bmdv.bund.de/SharedDocs/EN/publications/road-traffic-regulations-stvo.html', name: 'Turning (StVO §9)', desc: 'Check over your shoulder for cyclists before turning right. Yield to oncoming traffic when turning left.' },
      default: { url: 'https://www.bmdv.bund.de/SharedDocs/EN/publications/road-traffic-regulations-stvo.html', name: 'StVO', desc: 'Adapt your speed to visibility, weather, and traffic conditions. Drive defensively.' }
    }
  };
